...
export const useProductVariantStore = create<ProductVariantStore>()(
  persist(
    (set, get) => ({
      ...
      updateFormattedData: (data: Partial<FormattedData>) => {
        return set((state) => ({
          ...state,
          ...data,
        }));
      ...
      updateFormattedDataPrices: (prices: PricesVariantOption[], typeprice: string) =>
        set((state) => {
          return {
            ...state,
            prices: [...prices],
      ...
      bulkUpdateFormattedDataPrices: (
        ...
        set((state) => {
          // Add null check for formattedData
          if (!state.formattedData) return { formattedData: null };

          // Check if the current formattedData's id is in the selectedIds
          if (selectedIds.includes(state.formattedData.id)) {
            return {
              formattedData: {
                ...state.formattedData,
                prices: [...prices],
          ...
          return { formattedData: state.formattedData };
        }),
      ...
      batchUpdateAllFormattedDataPrices: (
        ...
        set((state) => {
          // Early return if formattedData is null
          if (!state.formattedData) {
            return { formattedData: null };
          }

          // Now TypeScript knows formattedData is not null
          return {
            formattedData: {
              ...state.formattedData,
              prices: [...(priceData[state.formattedData.id] ?? state.formattedData.prices ?? [])],
              typeprice: typeprice,
            },
          };
        }),
      ...
      getExistingPriceData: () => (state) => {
        ...
        // Add null check for formattedData
        if (!state.formattedData || !Array.isArray(state.formattedData)) {
      ...
      setFinalData: (data: FormattedData) =>
        set(() => ({
          finalData: data,
        })),
    ...
    {
      name: 'product-variant-add-option-store',
    }
  )
);
